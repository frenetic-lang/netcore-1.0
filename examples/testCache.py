...
# H1 0----1 S3 2----0 H2
basic = mininet.topolib.TreeTopo(depth=1, fanout=10)
...
def demo_learning_switch():
  ...
  while (4*(i+1)<len(runner.hosts)):
    h = runner.hosts[4*i]
    h.setIP("h%d-eth0"%(4*i+1), "%d.%d.%d.%d" % (100+i,100+i,100+i,100+i), 8)
    h = runner.hosts[4*i+1]
    h.setIP("h%d-eth0"%(4*i+2), "%d.%d.%d.%d" % (100+i,100+i,100+i, i), 8)
    h = runner.hosts[4*i+2]
    h.setIP("h%d-eth0"%(4*i+3), "%d.%d.%d.%d" % (100+i, 100+i , 0 , i), 8)
    h = runner.hosts[4*i+3]
    h.setIP("h%d-eth0"%(4*i+4), "%d.%d.%d.%d" % (100+i, 0 , 0 ,i), 8)
    i = i+1

    '''    if (i==0): 
      time.sleep(10)
      result = h.setIP("h1-eth0","10.10.10.2", 32)
      print "csdf" + result
      print h.IP()  
    else: 
      h.setIP("10.10.10.1", 32)
      print h.IP()  
    '''
  time.sleep(10)
